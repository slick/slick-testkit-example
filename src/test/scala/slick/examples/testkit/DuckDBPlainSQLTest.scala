...
class DuckDBPlainSQLTest extends PlainSQLTest {
  ...
  override def testInterpolation = ifCap(tcap.plainSql) {
    ...
    seq(
      ...
      sql"select id from USERS".as[Int] shouldYield Set(
        0,
        1,
        2,
        3
      ),                            // TODO Support `to` in Plain SQL Actions
      userForID(2).head shouldYield User(
        2,
        "guest"
  ...
  override def testEdgeTypes = ifCap(tcap.plainSql) {
    ...
    implicit val get: GetResult[Weird] =
      GetResult[Weird](r => new Weird(r.<<[Unit], r.<<, r.<<[Const.type]))

    implicit val setC = SetParameter.const[Const.type]
    implicit val setU = SetParameter[User] { (u, p) =>
      println(u); p.setInt(u.id); p.setString(u.name)
    }
    ...
    val create =
      sqlu"create table USERS2(ID int not null primary key, NAME varchar(255))"
        .map(_ shouldBe valueWhenReturnsNothing)
    ...
    def testGet =
      sql"select id, name from USERS2 where id = ${w.x._1}"
        .as[Weird]
        .map(_.head shouldBe w)
