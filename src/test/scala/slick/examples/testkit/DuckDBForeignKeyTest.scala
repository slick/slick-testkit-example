...
import com.typesafe.slick.testkit.tests.ForeignKeyTest

class DuckDBForeignKeyTest extends ForeignKeyTest {

  import tdb.profile.api.*

  override def testSimple = {
  ...
  override def testMultiColumn = {
    class A(tag: Tag) extends Table[(Int, Int, String)](tag, "a") {
      ...
      def * = (k1, k2, s)
      def bFK = foreignKey("b_fk", (k1, k2), bs)(b => (b.f1, b.f2), onDelete = ForeignKeyAction.NoAction)
    }
